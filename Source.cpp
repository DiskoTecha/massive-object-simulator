...
constexpr int object_amount = 450;

GravitationalBodySimulator gravSim;
MassiveObject objectsToMove[object_amount];
std::vector<NodePath> models; // Change to std::vector<NodePath>
Camera* camera;
...
{
    // Run the unit tests
    //runUnitTests();   // REMOVED FOR RELEASE
...
AsyncTask::DoneStatus moveSpheresTask(GenericAsyncTask* task, void* data)
{
    ...
    for (int i = 0; i < timesSpeed; i++)
    {
        // Get the indices of collided objects
        std::pair<std::vector<int>, std::vector<int>> collidedIndices = gravSim.step(dt);

        MassiveObject* objectsToMove = gravSim.getObjects();
        int objectAmount = gravSim.getObjectAmount();
        ...
        for (int index : collidedIndices.first)
        {
            float r = std::cbrt(3 * objectsToMove[index].getVolume() / (4 * 3.14159));
            models[index].set_scale(r, r, r);
        }


        for (int i = objectAmount; i < models.size(); i++)
        {
            models[i].remove_node();
        }
    }
...
void moveCamToCurrentObjectEvent(const Event* event, void* data)
{
    if (gravSim.getObjectAmount() > 0 && currentIndex >= 0 && currentIndex < gravSim.getObjectAmount())
    {
        MassiveObject* objects = gravSim.getObjects();
        Vector3 objPos = objects[currentIndex].getPosition();

        // Set the distance from the current object
        double distance = 1000.0; // You can adjust this distance as needed

        // Calculate the camera position
        Vector3 camPos = objPos - Vector3(distance, 0, 0);

        // Set the camera position and orientation
        cameraGroup.set_pos(camPos.x, camPos.y, camPos.z);
        cameraGroup.look_at(objPos.x, objPos.y, objPos.z);
    }
}

void leftArrowEvent(const Event* event, void* data)
{
    if (currentIndex > 0)
    {
        currentIndex--;
        moveCamToCurrentObjectEvent(event, data);
    }
}

void rightArrowEvent(const Event* event, void* data)
{
    if (currentIndex < gravSim.getObjectAmount() - 1)
    {
        currentIndex++;
        moveCamToCurrentObjectEvent(event, data);
    }
...
void speedUpEvent(const Event* event, void* data)
{
    timesSpeed = timesSpeed + 1;
    std::cout << "Times Speed = " << timesSpeed << std::endl;
}

void slowDownEvent(const Event* event, void* data)
{
    timesSpeed = timesSpeed == 0 ? 0 : timesSpeed - 1;
}
...
void runVisualizer(int argc, char** argv)
{
    ...
    models.reserve(object_amount); // Reserve space for the models
    ...
    for (int i = 0; i < object_amount; ++i)
    {
        ...
        models.push_back(visualizer->loadModel("models/sphere", Vector3(r, r, r), objectsToMove[i].getPosition()));
    }
    ...
    // Add space bar key event to move camera to the current object
    visualizer->addKeyEvent("space", "MoveToCurrentObject", moveCamToCurrentObjectEvent);

    // Add left and right arrow key events
    visualizer->addKeyEvent("arrow_left", "LeftArrowEvent", leftArrowEvent);
    visualizer->addKeyEvent("arrow_right", "RightArrowEvent", rightArrowEvent);

    // Add speed up slow down key events
    visualizer->addKeyEvent("arrow_up", "UpArrowEvent", speedUpEvent);
    visualizer->addKeyEvent("arrow_down", "DownArrowEvent", slowDownEvent);
